...
int main(int argc, char *argv[])
{
	...
	// Update voxel array combatant numbers
	if (print_scrn) std::cout << "\nUpdating combatant numbers...\n";
	for (int i=0; i<Simulator.NumVox(); i++){ // Update the combatant attributes
		// Write the combatant number to each voxel
		Simulator.VoxArray[i].CalculateCombatant();
	}
	...
	while (not Simulator.StopConditionMet())
	{
		...
		if (Step%100 == 0.0 && print_scrn) //Only output every n time steps
		{
			...
			std::cout << "IniCM1: " << Simulator.IniCM1.x << std::endl;
			std::cout << "IniCM2: " << Simulator.IniCM2.x << std::endl;
			std::cout << "IniCM1: " << Simulator.IniCM1.y << std::endl;
			std::cout << "IniCM2: " << Simulator.IniCM2.y << std::endl;
			std::cout << "IniCM1: " << Simulator.IniCM1.z << std::endl;
			std::cout << "IniCM2: " << Simulator.IniCM2.z << std::endl;
			std::cout << "CM1: " << Simulator.GetCombatantCM(1).x << std::endl;
			std::cout << "CM2: " << Simulator.GetCombatantCM(2).x << std::endl << std::endl;

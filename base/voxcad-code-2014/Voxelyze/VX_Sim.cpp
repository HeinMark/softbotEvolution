...
bool CVX_Sim::TimeStep(std::string* pRetMessage)
{
	...
	if (not CmInitialized)
	{
		IniCM1 = SS.CurCM1;
		IniCM2 = SS.CurCM2;
		//std::cout << "CM1 initialized to " << IniCM1.x << "," << IniCM1.y << "," << IniCM1.z << std::endl;
		//std::cout << "CM2 initialized to " << IniCM2.x << "," << IniCM2.y << "," << IniCM2.z << std::endl;
		CmInitialized = true;
	}
...
{
	//std::cout << "Calculating CM for combatant " << combatant << "\n";
	//bool test = false;
	vfloat TotalMass = 0;
	Vec3D<> Sum(0,0,0);
	int nVox = NumVox();
	for (int i=0; i<nVox; i++){
		CVXS_Voxel* it = &VoxArray[i];
		if (it->combatantNumber!=combatant){
			continue;
		} else {
			vfloat ThisMass = it->GetMass();
			Sum += it->GetCurPos()*ThisMass;
	...
	//std::cout << "Found voxels with this combatant: " << test << "\n";
